...
def verify():
    ...
    for event, data in ligo_events.items():
        ...
        omega_220_re = 1.5251
        f_uncorr = omega_220_re / (
            2 * math.pi * data["Mf"] * 4.926e-6
        )  # G=6.674e-11, c=3e8
    ...
    results["qnm_detectable"] = (
        qnm_correction > 1e-4
    )  # Will be > 10^-4 precision? Actually 8.4e-5 < 1e-4
...
print(
    f"  δ_eff = {r['delta_eff']:.6f} ≈ 1/{r['delta_eff_inv']:.0f} (dev from 1/1200: {r['dev_1200_pct']:.3f}%)"
)
...
print("\n── PART 2: 4D Extension ──")
print(
    f"  K3 Betti: b₂ = {r['K3_b2']} = h^(1,1) + 2h^(2,0) = {r['K3_hodge_11']} + 2×{r['K3_hodge_20']} = {r['K3_b2_check']}"
)
...
print(
    f"  Best rational approx with q < 1200: {r['best_rational_approx']} (dev: {r['best_rational_dev_pct']:.3f}%)"
)
print(f"  No better approx below 1200: {r['no_better_approx_below_1200']}")
for k in [20, 21, 23, 24]:
    print(
        f"  b₂ = {k}: dev from 1/1200 = {r[f'dev_b2_{k}_pct']:.3f}% → {'INCOMPATIBLE' if r[f'b2_{k}_incompatible'] else 'compatible'}"
    )
print(
    f"  Spin structures: {r['spin_structures_total']} total, {r['spin_structures_even']} even (Arf=0), {r['spin_structures_good_pct']:.1f}% give δ_eff ≈ 1/1200"
)
for eps in [0.001, 0.01, 0.05]:
    print(
        f"  Stability: |ε| = {eps} → dev from 1/1200 = {r[f'stability_eps_{eps}']:.3f}%"
    )
...
for name in ["Klein", "Macbeath", "Hurwitz_3"]:
    s = r[f"hurwitz_{name}"]
    print(
        f"  {name}: g={s['g']}, n={s['n']}, b₂={s['b2']}, δ_eff = {s['delta_eff']:.6e}, γ = {s['gamma']:.6e}"
    )
    ...
    with open(_output_path, "w") as f:
        ...
        for k, v in r.items():
            if isinstance(v, dict):
                clean[k] = {
                    kk: (vv if not isinstance(vv, dict) else str(vv))
