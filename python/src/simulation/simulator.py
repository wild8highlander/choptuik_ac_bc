...
class Simulator:
    ...
    def sweep_delta_C(
        self,
        delta_C_range: tuple[float, float] | None = None,
        n_points: int = 200,
        ...
        deviation_arr = (
            np.abs(delta_ch_full_arr - self.delta_obs) / self.delta_obs * 100
        )
        ...
        self._log(
            f"Delta_C sweep: {n_points} points in [{delta_C_range[0]}, {delta_C_range[1]}]"
        )
    ...
    def sweep_lambda_1(
        self,
        lam_range: tuple[float, float] = (2.0, 6.0),
        n_points: int = 200,
    ...
    def convergence_analysis(
        ...
        for ps in partial_sums:
            self._log(
                f"  Order {ps['order']}: Δ={ps['value']:.6f}, dev={ps['deviation']:.3f}%"
            )
