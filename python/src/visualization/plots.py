...
class PlotGenerator:
    ...
    def spinor_phase_diagram(
        self, delta_A: float, delta_B: float, delta_C: float
    ) -> list[str]:
    ...
    def eigenvalue_landscape(self, sweep_data: dict) -> list[str]:
        ...
        ax1.axvline(
            x=np.pi / 7, color="gray", linestyle=":", alpha=0.7, label=r"$\pi/7$"
        )
    ...
    def qnm_comparison(self, predictions: list[dict]) -> list[str]:
        ...
        ax1.bar(
            names, delta_f, color="#e74c3c", alpha=0.7, label=r"$\Delta f$ (predicted)"
        )
    ...
    def deviation_analysis(self, choptyuk_data: dict) -> list[str]:
        ...
        for bar, val in zip(bars, deviations, strict=True):
            ax.text(
    ...
    def convergence_diagram(self, convergence_data: dict) -> list[str]:
        ...
        ax2.semilogy(
            orders, [d + 1e-10 for d in devs], "ro-", linewidth=2, markersize=6
        )
    ...
    def lambda_1_sweep(self, sweep_data: dict) -> list[str]:
        ...
        ax1.axhline(y=3.443, color="g", linestyle="--", label=r"$\Delta_{obs}$")
        ax1.axvline(
            x=3.838, color="gray", linestyle=":", alpha=0.7, label=r"$\lambda_1=3.838$"
        )
    ...
    def surface_comparison(self, surfaces_data: list[dict]) -> list[str]:
        ...
        w = 0.35
        ax.bar(
            x - w / 2, delta_bc, w, label=r"$\Delta_{bC}$", color="#3498db", alpha=0.8
        )
        ax.bar(
            x + w / 2, delta_ch, w, label=r"$\Delta_{Ch}$", color="#e74c3c", alpha=0.8
        )
    ...
    def generate_all(self, results: dict, sim_results: dict | None = None) -> list[str]:
        ...
        if ph:
            all_paths.extend(
                self.spinor_phase_diagram(ph["delta_A"], ph["delta_B"], ph["delta_C"])
            )
    ...
    def generate_enhanced_plots(
        self, enhanced_results: dict | None = None
    ) -> list[str]:
        ...
        if qnm:
            ...
            if events:
                ...
                ax.set_title(
                    "Enhanced QNM Frequency Shifts (Spinorial Braking)", fontsize=13
                )
                ax.grid(True, alpha=0.3, axis="y")
        ...
        if k3:
            ...
            for i, v in enumerate(betti_vals):
                if v > 0:
                    ax.text(
                        i, v + 0.3, str(v), ha="center", fontsize=12, fontweight="bold"
                    )
            ax.grid(True, alpha=0.3, axis="y")
        ...
        if tyuk:
            ...
            if delta_corr > 0:
                ...
                ax.set_title(
                    "Tyukovsky Echo Period: Original vs Corrected", fontsize=13
                )
                shift_pct = (T_corr - T_0) / T_0 * 100
                ax.annotate(
                    f"Shift: {shift_pct:.2f}%",
                    xy=(1, T_corr),
                    xytext=(1.3, T_corr),
                    fontsize=11,
                    color="#e74c3c",
        ...
        if criticism:
            ...
            if b2_uniq:
                ...
                colors = [
                    "#2ecc71" if b2_uniq[k]["compatible"] else "#e74c3c" for k in ks
                ]
                ...
                ax.set_title(
                    "Uniqueness of $b_2 = 22$ for $\\delta_{eff} \\approx 1/1200$",
                    fontsize=13,
                )
                ax.legend(fontsize=10)
                ax.grid(True, alpha=0.3, axis="y")
        ...
        if spin_struct:
            ...
            ax.pie(
                [even, odd],
                labels=[f"Even (Arf=0): {even}", f"Odd (Arf=1): {odd}"],
