...
@testset "Klein Curve Properties" begin
    kc = KleinCurve()
    # NOTE: genus / aut_order / R are FIELDS of KleinCurve (there are no
    # genus(kc) / automorphism_order(kc) / scalar_curvature(kc) functions).
    @test kc.genus == 3
    @test kc.aut_order == 168
    @test kc.R ≈ -2 atol=TOL_STRICT
    @test is_hurwitz_curve(kc)
end

@testset "Spinor Phases" begin
    sp = SpinorPhases()
    # NOTE: delta_A / delta_B / delta_C are FIELDS of SpinorPhases.
    @test sp.delta_A ≈ π/2 atol=TOL_STRICT
    @test sp.delta_B ≈ π/3 atol=TOL_STRICT
    @test sp.delta_C ≈ π/7 atol=TOL_STRICT
    @test sp.delta_A > sp.delta_B > sp.delta_C
end

@testset "Dirac Operator" begin
    d = DiracOperator()
    # NOTE: lambda_D2_triv is a FIELD of DiracOperator.
    @test d.lambda_D2_triv ≈ REF_LAMBDA1_DIRAC atol=TOL_LOOSE
end

@testset "Choptyuk Formula" begin
    cf = ChoptyukFormula()
    # Real API: bC_correction / aC_correction / choptyuk_constant functions;
    # b_Ch is a FIELD of ChoptyukFormula.
    @test bC_correction(cf) ≈ REF_DELTA_BC atol=TOL_LOOSE
    @test aC_correction(cf) ≈ REF_DELTA_CH_BASE atol=TOL_LOOSE
    @test choptyuk_constant(cf) ≈ REF_DELTA_CH_FULL atol=TOL_LOOSE
    @test cf.b_Ch ≈ REF_B_CH atol=TOL_LOOSE
end
...
@testset "Deviations Within Tolerance" begin
    cf = ChoptyukFormula()
    observed = 3.443
    dev_bc = abs(bC_correction(cf) - observed) / observed * 100
    dev_full = abs(choptyuk_constant(cf) - observed) / observed * 100
    @test dev_bc < 0.2
    @test dev_full < 0.2
end
