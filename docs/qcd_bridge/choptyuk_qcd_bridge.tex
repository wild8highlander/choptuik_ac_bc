...
Originally this value was an \emph{empirical input} to the present
framework, measured from Choptuik-type collapse simulations --- exactly
as $\alpha_s(M_Z) \approx 0.118$ is an empirical input to QCD, measured
from $R$-ratio data.  The $K3$ topology predicts the \emph{existence} of
a log-periodic modulation and fixes the discrete scale invariance ratio
$\lambda_{\mathrm{DSI}} = 22$ and the Euler characteristic $\chi = 24$.
\emph{Editorial verification (September 2026, folder
\texttt{audit\_transfer/}, task~2) has since closed this input at
leading order}: the DSI cascade with $\lambda = 22$ fixes the bare
amplitude as the framework's own constant
$b_{Ch}(22) = 1 - \cos(2\pi/22) = 0.04051$ (zero fitted parameters;
$+0.82\%$ from the measured value), and the braking-coupled amplitude RG
map gives $b_{Ch}\bigl(22(1+\gamma)\bigr) = 0.04036$ with
$\gamma = \delta_C^4/22$ ($+0.45\%$).  The remaining $0.8\%$ deficit is
reproduced numerically as the finite-window systematics of the
harmonic-fitting protocol.  The structure of the modulation ---
existence, period $\omega = 2\pi/\ln 22$, and bare amplitude
$b_{Ch}(22)$ --- is therefore fixed by topology; only the last digit of
the measured value carries window systematics.  See
\texttt{audit\_transfer/README.md}, \S5 and the PDF appendix
``Audit and Transfer''.
\end{caveat}
...
\midrule
$K3$ topology       & $c_{K3}$  & $0.04018$ & $b_2(K3) = 22$, $\chi = 24$ & derived (leading order; see \texttt{audit\_transfer/}) \\
...
\midrule
$c_{K3}$   & $0.04018$ & derived (leading order) \\
